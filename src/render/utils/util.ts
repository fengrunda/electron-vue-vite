 ...
 */
export function formatAmount (amount: string | number, style = 'currency', currency = 'CNY', decimal = 2): string {
  ...
  try { text = amountNum.toLocaleString('zh', params) } catch (error) {
    console.error(error)
  }
  if (!text) {
 ...
 * @returns {any}
 */
export function removeEmpty (obj: any, checkString = false) {
